...
import { type ListData, type StaffListItem, type StudentListItem } from "../components/assistant/ListCard";
import {
  parseAxpoIntent,
  detectListIntent,
  isLLMAvailable,
...
const SUGGESTIONS = [
  { icon: <Users className="h-3.5 w-3.5" />, label: "List Students", prompt: "Show all students" },
  { icon: <Users className="h-3.5 w-3.5" />, label: "List Teachers", prompt: "List all teachers and staff" },
  { icon: <BarChart3 className="h-3.5 w-3.5" />, label: "Monthly Fees", prompt: "Show monthly fees collected" },
  { icon: <DollarSign className="h-3.5 w-3.5" />, label: "Monthly Salary", prompt: "Show monthly salary paid" },
...
export function AxpoAssistantPage() {
    ...
    (query: ParsedAnalyticsQuery): AnalyticsData | null => {
      ...
      switch (query.queryType) {
        ...
        case "monthly_salary_report": {
          // Get all months with salary payments
        ...
        switch (intentResult.intent) {
          case "add_student": {
            const studentPayload = (items as Record<string, unknown>[]).map((studentData: Record<string, unknown>) => ({
              ...
              feeType: ((studentData.feeType as string) || "Regular") as FeeType,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      // Resolve data for CRUD operations
      ...
      // For other CRUD operations, show form
