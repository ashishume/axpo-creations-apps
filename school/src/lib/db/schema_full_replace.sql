...
-- ============================================
-- Run this file to DROP all teaching tables and recreate from scratch.
...
CREATE POLICY "Users see own org or all if platform admin"
  ...
  USING (id = current_organization_id() OR current_organization_id() IS NULL);
...
CREATE POLICY "Platform admin can insert orgs"
  ...
  WITH CHECK (current_organization_id() IS NULL);
...
CREATE POLICY "Sessions by org"
  ...
  USING (
    school_id IN (
      ...
      WHERE organization_id = current_organization_id() OR current_organization_id() IS NULL
    )
  )
  WITH CHECK (
    school_id IN (
...
CREATE POLICY "Classes by org"
  ...
  USING (
    session_id IN (
      ...
      WHERE sc.organization_id = current_organization_id() OR current_organization_id() IS NULL
    )
  )
  WITH CHECK (
    session_id IN (
...
CREATE POLICY "Stocks by org"
  ON stocks FOR ALL TO authenticated
  USING (
    session_id IN (
      SELECT s.id FROM sessions s
      JOIN schools sc ON sc.id = s.school_id
...
CREATE POLICY "Salary payments by org"
  ON salary_payments FOR ALL TO authenticated
  USING (
    staff_id IN (
...
CREATE POLICY "Fixed monthly costs by org"
  ON fixed_monthly_costs FOR ALL TO authenticated
  USING (
    session_id IN (
      SELECT s.id FROM sessions s
      JOIN schools sc ON sc.id = s.school_id
...
BEGIN
  FOR tbl IN
    SELECT unnest(ARRAY[
      'roles', 'users', 'organizations', 'schools',
