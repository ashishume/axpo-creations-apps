...
const legalRoutes = JSON.parse(readFileSync(new URL('./vercel.json', import.meta.url), 'utf8')).rewrites
  .filter((route: { source: string }) => route.source.startsWith('/figureout')) as { source: string; destination: string }[]
...
export default defineConfig({
  plugins: [{ name: 'figureout-static-pages', configureServer(server) { server.middlewares.use(serveLegal) },
